...
client.login('NTY3MDA1OTY5ODU1MjgzMjMw.XLNaEw.dQxGs83SfGVhQvXuJUUTtjzliHQ');
 
client.on('message', message =>{
    if(message.content === "r!help"){
        message.channel.sendMessage('```Commandes Bot : r!kick, r!ban, r!mute, r!clear Bot created by ZeWolf929```');
...
client.on("message", message => {
    ...
    if (args[0].toLowerCase() === prefix + "mute") {
        ...
        let muterole = message.guild.roles.find(role => role.name === 'Réduit au silence')
        if (muterole) {
            member.addRole(muterole)
            message.channel.send(member + ' a été mute :white_check_mark:')
        }
        else {
            message.guild.createRole({name: 'Réduit au silence', permissions: 0}).then((role) => {
